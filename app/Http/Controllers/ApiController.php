...
class ApiController extends Controller
{
  ...
  {
    try {
      $duplicate = DB::table('book_titles')->where('book_title_name', $req->book_title_name)->get();
      if (count($duplicate)) {
        return response()->json('Duplicate Data Entry');
      } else {
      ...
      for ($i = 1; $i <= $d; $i++) {
        ...
        $data = DB::select(
          ...
          [
            ...
            $req->publish_year, $req->no_of_pages, $isbn[$i - 1], $req->language, $req->series, $req->source, $req->content, $req->currency_id, $req->document_id, $req->subject_id, $req->vendor_id, $req->bill_id, $req->suppl_copies, $req->abstract, $req->nature_of_binding, $req->notes, $req->keywords, $req->call_no, $req->book_price, $book_image, $req->college_id, $req->location_id, $req->entry_by
          ]
        );
        $res = DB::select('SELECT @result AS res');
      }
  ...
  public function last_accession_number($clg_id)
  {
    try {
      ...
      if ($data == null) {
        return response()->json($shortName[0]->short_name . '1');
      } else {
...
where
  ...
  public function book_dtls_btw_access_no_range($from, $to)
  {
    try {
      // $data = DB::table('books')
      //   ->join('book_titles', 'books.book_title_id', '=', 'book_titles.book_title_id')
      ...
      //   ->join('document_types', 'books.document_id', '=', 'document_types.document_id')
      //   // ->join('subjects', 'books.subject_id', '=', 'subjects.subject_id')
      //   ->join('vendors', 'books.vendor_id', '=', 'vendors.vendor_id')
      //   ->join('bill_details', 'books.bill_id', '=', 'bill_details.bill_id')
      //   ->join('colleges', 'books.college_id', '=', 'colleges.college_id')
      //   ->join('book_locations', 'books.location_id', '=', 'book_locations.location_id')
      //   ->whereBetween('accession_no', [$from, $to])->get();
      $data = DB::select("call book_dtls_btw_access_no_range(?,?)", array($from, $to));
      return response()->json($data);
    } catch (\Exception $e) {
      // Exception handling
      return response()->json(['error' => $e->getMessage()], 500);
  ...
  {
    try {
      $duplicate = DB::table('subjects')->where('subject_name', $req->subject_name)->get();
      if (count($duplicate)) {
        return response()->json('Duplicate Data Entry');
      } else {
